...
export default function QAChat() {
  ...
  return (
    <div className="w-full h-full flex flex-col">
      <div className="bg-slate-900/70 rounded-2xl border border-slate-800 flex flex-col min-h-[450px] backdrop-blur-sm">
        <div className="px-4 py-3 border-b border-slate-800/80 bg-slate-900/80">
          <h2 className="font-heading font-semibold text-sm text-slate-100 flex items-center gap-2">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-8">
              ...
              <p className="font-medium text-slate-100 mb-1">No messages yet</p>
            ...
            messages.map((msg, idx) => (
              <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`
                  ...
                  ${msg.role === 'user' ? 'bg-primary text-white' : 'bg-slate-800 text-primary'}
                `}>
                  {msg.role === 'user' ? <User className="w-4 h-4" /> : <Bot className="w-4 h-4" />}
                </div>
                <div className={`
                  max-w-[80%] rounded-lg p-3
                  ${msg.role === 'user' ? 'bg-primary text-white' : 'bg-slate-800 text-slate-100'}
                `}>
                  {msg.role === 'user' ? (
          ...
          {isLoading && (
            <div className="flex gap-3">
              <div className="w-8 h-8 rounded-lg bg-slate-800 text-primary flex items-center justify-center">
          ...
          {error && (
            <div className="flex items-center gap-2 p-2 bg-red-900/40 text-red-200 rounded text-sm">
        ...
        <form onSubmit={handleSubmit} className="p-3 border-t border-slate-800/80 bg-slate-900/80">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 rounded-lg border border-slate-700 bg-slate-900/60 text-sm text-slate-100 placeholder:text-slate-500 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
              disabled={isLoading}
            />
            <button
              type="submit"
